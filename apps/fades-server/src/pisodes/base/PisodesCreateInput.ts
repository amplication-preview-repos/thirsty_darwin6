/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsDate,
  IsOptional,
  ValidateNested,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { RediffusionsCreateNestedManyWithoutPisodesItemsInput } from "./RediffusionsCreateNestedManyWithoutPisodesItemsInput";

@InputType()
class PisodesCreateInput {
  @ApiProperty({
    required: false,
  })
  @IsDate()
  @Type(() => Date)
  @IsOptional()
  @Field(() => Date, {
    nullable: true,
  })
  dateDeDiffusion?: Date | null;

  @ApiProperty({
    required: false,
    type: () => RediffusionsCreateNestedManyWithoutPisodesItemsInput,
  })
  @ValidateNested()
  @Type(() => RediffusionsCreateNestedManyWithoutPisodesItemsInput)
  @IsOptional()
  @Field(() => RediffusionsCreateNestedManyWithoutPisodesItemsInput, {
    nullable: true,
  })
  rediffusionsItems?: RediffusionsCreateNestedManyWithoutPisodesItemsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  rSum?: string | null;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  titre?: string | null;
}

export { PisodesCreateInput as PisodesCreateInput };
